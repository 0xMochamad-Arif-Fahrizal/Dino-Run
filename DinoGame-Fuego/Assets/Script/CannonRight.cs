...
public class CannonRight : MonoBehaviour
{
    ...
    // Mengecek apakah pemain berada dalam jarak deteksi di sebelah kanan dan sejajar horizontal
    bool PlayerInRightRange()
    {
        Collider2D player = Physics2D.OverlapCircle(transform.position, detectionRange, playerLayer);
        if (player != null)
        {
            // Pastikan pemain berada di sebelah kanan cannon dan sejajar secara vertikal
